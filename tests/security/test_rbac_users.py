        ...
        for method, path in self.ADMIN_ENDPOINTS:
            ...
            if method in ("PATCH", "POST"):
                json_body = (
                    {"first_name": "X"}
                    if "reset" not in url
                    else {"new_password": "TestPass2"}
                )
            resp = await self._request(
                client, method, url, make_auth_header(servant_user), json_body
            )
            assert (
                resp.status_code == 403
            ), f"SERVANT should get 403 on {method} {url}, got {resp.status_code}"

    async def test_parent_cannot_access_admin_endpoints(
        self, client: AsyncClient, parent_user: User, admin_user: User
    ):
        uid = str(admin_user.id)
        for method, path in self.ADMIN_ENDPOINTS:
            url = path.replace("{uid}", uid)
            ...
            resp = await self._request(
                client, method, url, make_auth_header(parent_user), json_body
            )
            assert (
                resp.status_code == 403
            ), f"PARENT should get 403 on {method} {url}, got {resp.status_code}"

    async def test_unauthenticated_cannot_access_any_user_endpoint(
        ...
        for method, url in endpoints:
            ...
            ), f"Unauthenticated should get 401 on {method} {url}, got {resp.status_code}"
...
class TestSelfServiceIsolation:
    ...
    async def test_servant_can_view_own_profile(
    ...
    async def test_parent_can_update_own_profile(
    ...
    async def test_inactive_user_cannot_access(
        ...
        resp = await client.get(
            "/api/v1/users/me", headers=make_auth_header(inactive_user)
        )
