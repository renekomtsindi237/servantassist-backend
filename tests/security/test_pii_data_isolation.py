...
async def _insert_user(session, first_name="Jean", last_name="Ndaa",
                       email=None, phone=None) -> User:
...
class TestSundayAssignmentTableHasNoPII:
    ...
    async def test_servant_name_not_plaintext(self, db_session):
        ...
        template = await schedule_repo.create_template(SundayScheduleTemplate(
            id=uuid4(),
        ...
        mass = await schedule_repo.create_mass(SundayMassSlot(
            id=uuid4(),
        ...
        assignment = await schedule_repo.create_assignment(SundayMassAssignment(
            id=uuid4(),
