...
# Login admin
st, body = POST("/auth/login", {"username": "renekomtsindi7@gmail.com", "password": "Mbetoumou olive77"},
                ct="application/x-www-form-urlencoded")
ADMIN_TOKEN = body.get("access_token", "")
test("Login admin valide", st == 200 and bool(ADMIN_TOKEN), st,
     f"token={'ok' if ADMIN_TOKEN else 'absent'}")

# Login mauvais mot de passe (small delay to avoid rate limiting)
time.sleep(0.5)
st, _ = POST("/auth/login", {"username": "renekomtsindi7@gmail.com", "password": "mauvais_mdp_xyz"},
             ct="application/x-www-form-urlencoded")
test("Login mauvais MDP → 401", st == 401, st)

# Login email inexistant
time.sleep(0.5)
st, _ = POST("/auth/login", {"username": "inconnu_xyz@bmra.cm", "password": PASS},
             ct="application/x-www-form-urlencoded")
...
st, _ = POST("/auth/reset-password", {"token": "faux-token-invalide", "new_password": "Nouveau1234!"})
...
st, _ = PATCH("/users/me", {"first_name": me.get("first_name", ""), "last_name": me.get("last_name", "")}, ADMIN_TOKEN)
...
st, _ = PATCH("/users/me/password", {
    "current_password": "Mbetoumou olive77",
    "new_password": "Mbetoumou olive77"
}, ADMIN_TOKEN)
...
test("GET /users/ (liste paginée)", st == 200 and "items" in users, st,
...
test("POST /auth/register servant", st == 201 and bool(SERVANT_ID), st,
...
test("GET /admin/invitations", st == 200, st, f"count={len(invites) if isinstance(invites, list) else '?'}")

st, inv = POST("/admin/invitations", {
    "role": "PARENT",
    "parent_name": "Marie Dupont",
    "notes": "Parent test E2E"
}, ADMIN_TOKEN)
INV_ID = inv.get("id", "")
INV_CODE = inv.get("code", "")
test("POST /admin/invitations (créer code)", st == 201 and bool(INV_CODE), st,
     f"code={INV_CODE}")
...
if INV_CODE:
    ...
    test("Register parent avec code invitation", st == 201 and bool(PARENT_ID), st,
...
test("GET /dashboard/summary (admin)", st == 200 and "total_servants" in summary, st,
...
test("GET /reports/ (liste)", st == 200 and "items" in reports, st,
...
test("GET /events/ (liste)", st == 200 and "items" in evts, st,
...
EVT_ID = evt.get("id", "")
test("POST /events/ (créer événement)", st == 201 and bool(EVT_ID), st,
...
test("GET /material/items/ (liste)", st == 200 and "items" in items, st,
     f"items={items.get('total', 0)}")

# POST sans slash final (évite le 307 redirect)
st, item = POST("/material/items", {
...
ITEM_ID = item.get("id", "")
test("POST /material/items (créer article)", st == 201 and bool(ITEM_ID), st,
...
test("POST /training/sessions (admin)", st == 201 and bool(TRAINING_ID), st,
     f"id={'ok' if TRAINING_ID else 'absent'}")

if TRAINING_ID:
    st, _ = GET(f"/training/sessions/{TRAINING_ID}", SERVANT_TOKEN if SERVANT_TOKEN else ADMIN_TOKEN)
...
if SERVANT_ID:
    ...
    CASE_ID = case.get("id", "")
    test("POST /discipline/ (créer cas)", st == 201 and bool(CASE_ID), st,
...
test("GET /sport-culture/events (liste)", st == 200 and "items" in sc_evts, st,
     f"items={sc_evts.get('total', 0)}")

st, sc_evt = POST("/sport-culture/events", {
...
test("POST /sport-culture/events (créer événement)", st == 201 and bool(SC_EVT_ID), st,
...
test("GET /communication/me (mes notifications)", st == 200, st,
...
test("POST /api-keys/ (créer clé)", st == 201 and bool(RAW_KEY), st,
...
cleaned = []
for user_id, label in [(SERVANT_ID, "servant"), (PARENT_ID if 'PARENT_ID' in dir() else "", "parent")]:
    if user_id:
