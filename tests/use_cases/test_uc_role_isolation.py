...
class TestLoginMethodIsolation:
    ...
    async def test_admin_cannot_login_by_phone(self, client: AsyncClient, admin_user: User):
    ...
    async def test_servant_cannot_login_by_email(self, client: AsyncClient, servant_user: User):
    ...
    async def test_parent_cannot_login_by_email(self, client: AsyncClient, parent_user: User):
    ...
    async def test_aumonier_cannot_login_by_phone(self, client: AsyncClient, aumonier_user: User):
...
class TestAdminEndpointIsolation:
    ...
    async def test_servant_cannot_list_invitations(self, client: AsyncClient, servant_user: User):
        headers = make_auth_header(servant_user)
        resp = await client.get("/api/v1/admin/invitations", headers=headers)
        assert resp.status_code == 403

    async def test_parent_cannot_create_invitation(self, client: AsyncClient, parent_user: User):
    ...
    async def test_admin_can_access_admin_endpoints(self, client: AsyncClient, admin_user: User):
