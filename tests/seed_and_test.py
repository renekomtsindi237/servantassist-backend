...
def check(name: str, passed: bool, status: int, detail: str = ""):
    ...
    print(f"  {icon} {color}[{status}]{RESET} {name}" +
...
check("GET /health → serveur actif", st == 200, st,
      body.get("status", "unreachable"))
if st != 200:
...
# Login admin (email)
st, body = POST("/auth/login",
                {"username": ADMIN_EMAIL, "password": ADMIN_PASS},
                ct="application/x-www-form-urlencoded")
ADMIN_TOKEN = body.get("access_token", "")
ADMIN_REFRESH = body.get("refresh_token", "")
check("Login admin (email) → 200 + token", st == 200 and bool(ADMIN_TOKEN), st,
      "token=ok" if ADMIN_TOKEN else "token absent")

time.sleep(0.3)
st, me_body = GET("/users/me", ADMIN_TOKEN)
ADMIN_ID = me_body.get("id", "")
check("GET /users/me (admin) → 200", st == 200 and bool(ADMIN_ID), st,
      me_body.get("email", "?"))

# Mauvais mot de passe
time.sleep(0.4)
st, _ = POST("/auth/login", {"username": ADMIN_EMAIL, "password": "mauvais_xyz"},
             ct="application/x-www-form-urlencoded")
check("Login mauvais MDP → 401", st == 401, st)

# Email inconnu
time.sleep(0.4)
st, _ = POST("/auth/login", {"username": "ghost_xyz@bmra.cm", "password": NEW_PASS},
             ct="application/x-www-form-urlencoded")
...
if ADMIN_REFRESH:
    ...
    check("POST /auth/refresh → 200", st == 200, st,
...
SERVANT_NAMES = [
    ("Jean", "Mbarga"),   ("Paul", "Atangana"), ("Pierre", "Nkolo"),
    ("Marie", "Tamba"),   ("Joseph", "Enyegue"), ("Luc", "Ondoua"),
    ("Marc", "Beyeme"),   ("Anne", "Abena"),    ("Thomas", "Mvondo"),
    ("Clara", "Fouda"),   ("David", "Essama"),   ("Sophie", "Biyong"),
    ("André", "Nanga"),   ("Rachel", "Owona"),   ("Samuel", "Minko"),
]
...
check(f"Création 15 servants", len(SERVANTS) >= 10, 201,
      f"{len(SERVANTS)}/15 créés")

# Login de quelques servants (via phone)
for srv in SERVANTS[:5]:
    time.sleep(0.2)
    st, tok_body = POST("/auth/login/phone",
                        {"phone_number": srv["phone"], "password": NEW_PASS})
    if st == 200 and tok_body.get("access_token"):
        SERVANT_TOKENS.append(tok_body["access_token"])
check(f"Login servants (phone) → tokens", len(SERVANT_TOKENS) >= 3, 200,
...
check(f"Création parents avec codes invitation", len(PARENTS) >= 2, 201,
...
check("GET /users/ paginé → 200", st == 200 and "items" in users_page, st,
...
if SERVANT_ID_1:
    ...
    check(f"GET /users/{{id}} (détail servant)", st == 200, st,
          srv_detail.get("first_name", "?"))

    # Modifier profil
    st, _ = PATCH(f"/users/{SERVANT_ID_1}",
                  {"first_name": SERVANTS[0]["first_name"],
                   "last_name": "MisàJour"}, ADMIN_TOKEN)
    check("PATCH /users/{id} (modifier profil)", st == 200, st)

    # Reset password par admin
    st, _ = POST(f"/users/{SERVANT_ID_1}/reset-password",
                 {"new_password": NEW_PASS}, ADMIN_TOKEN)
...
st, _ = PATCH("/users/me/password",
              {"current_password": ADMIN_PASS,
               "new_password": ADMIN_PASS}, ADMIN_TOKEN)
...
for poste, srv in poste_assignments:
    if not srv:
        continue
    time.sleep(0.15)
    st, nom = POST("/responsables/nominations", {
        "user_id": srv["id"],
        "poste": poste,
    }, ADMIN_TOKEN)
    if st in (200, 201) and nom.get("id"):
        ...
        if st2 == 200:
            nom_list = (all_noms if isinstance(all_noms, list)
                        else all_noms.get("items", []))
            for n in nom_list:
                if str(n.get("poste", "")) == poste and n.get("user_id"):
                    ...
                    if st3 == 200:
                        ...
                        POST(f"/users/{n['user_id']}/reset-password",
                             {"new_password": NEW_PASS}, ADMIN_TOKEN)
                        POSTE_MAP[poste] = {
                            **u,
...
check(f"Nominations postes ({len(POSTE_MAP)} créées)", len(POSTE_MAP) >= 8, 201,
...
check("GET /responsables/nominations (liste)", st == 200, st,
...
check("GET /responsables/postes", st == 200, st,
...
check("GET /admin/invitations (liste)", st == 200, st,
      f"count={len(inv_list) if isinstance(inv_list, list) else '?'}")

st, inv = POST("/admin/invitations",
               {"role": "PARENT", "parent_name": "Test E2E", "notes": "test"}, ADMIN_TOKEN)
INV_ID = inv.get("id", "")
INV_CODE = inv.get("code", "")
check("POST /admin/invitations → 201", st == 201 and bool(INV_CODE), st,
...
check("GET /dashboard/summary (admin) → 200", st == 200, st,
...
EVENT_IDS = []
events_data = [
    {"title": f"Messe Dominicale {RUN_ID} #1",  "event_type": "MESSE_DOMINICALE",
     "start_time": FUTURE_1,     "end_time": FUTURE_1_END,
     "location": "Paroisse Notre-Dame"},
    {"title": f"Messe de Semaine {RUN_ID}",      "event_type": "MESSE_SEMAINE",
     "start_time": FUTURE_2,     "end_time": FUTURE_2_END,
     "location": "Cathédrale"},
    {"title": f"Messe Pontificale {RUN_ID}",     "event_type": "MESSE_PONTIFICALE",
     "start_time": FUTURE_3,     "end_time": FUTURE_3_END,
     "location": "Chapelle"},
    {"title": f"Récollection {RUN_ID}",          "event_type": "RECOLLECTION",
     "start_time": FUTURE_4,     "end_time": FUTURE_4_END,
     "location": "Salle paroissiale"},
    {"title": f"Répétition passée {RUN_ID}",     "event_type": "REPETITION",
     "start_time": PAST_1,       "end_time": PAST_1_END,
...
check(f"Création {len(events_data)} événements", len(EVENT_IDS) >= 4, 201,
      f"{len(EVENT_IDS)}/{len(events_data)} créés")

# Liste filtrée
st, evts = GET("/events/?page=1&page_size=10", ADMIN_TOKEN)
check("GET /events/ (liste paginée)", st == 200 and "items" in evts, st,
      f"total={evts.get('total', 0)}")

if EVENT_IDS:
    # Détail
    st, ev_detail = GET(f"/events/{EVENT_IDS[0]}", ADMIN_TOKEN)
    check("GET /events/{id} → 200", st == 200, st,
          ev_detail.get("title", "?"))

    # Modifier
    st, _ = PATCH(f"/events/{EVENT_IDS[0]}",
                  {"title": "Messe Ordinaire #1 — modifiée", "status": "PUBLIE"}, ADMIN_TOKEN)
    ...
    # Participants
    if SERVANT_ID_1:
        st, _ = POST(f"/events/{EVENT_IDS[0]}/participants",
                     {"user_id": SERVANT_ID_1}, ADMIN_TOKEN)
...
if EVENT_IDS and SERVANTS:
    ...
    check("POST /assignments/batch (lot)", st in (200, 201, 422), st,
...
if ASSIGNMENT_IDS and SERVANT_TOKEN_1:
    ...
    st, _ = PATCH(f"/assignments/{asgn_id}/my-status",
                  {"status": "ACCEPTED"}, SERVANT_TOKEN_1)
    check("PATCH /assignments/{id}/my-status (accepter)", st in (200, 403, 404), st)

# Marquer présence (query param, pas JSON body)
if ASSIGNMENT_IDS:
    st, _ = _req("PATCH", f"{BASE}/assignments/{ASSIGNMENT_IDS[0]}/presence?present=true",
                 token=ADMIN_TOKEN)
...
for i in range(4):
    ...
    st, sess = POST("/attendance-sessions/", {
...
check(f"Création 4 sessions d'appel", len(SESSION_IDS) >= 2, 201,
      f"{len(SESSION_IDS)}/4 créées")

# Lister
st, sess_list = GET("/attendance-sessions/?page=1&page_size=10", ADMIN_TOKEN)
check("GET /attendance-sessions/ (liste)", st == 200, st,
...
if SESSION_IDS:
    ...
    for srv in SERVANTS[:8]:
        time.sleep(0.1)
        st, _ = POST(f"/attendance-sessions/{SESSION_IDS[0]}/records", {
            "servant_id": srv["id"],
    ...
    check(f"POST /attendance-sessions/{SESSION_IDS[0]}/records ({records_added} présences)",
          records_added >= 5, 201, f"{records_added}/8 enregistrés")
    ...
    if len(SESSION_IDS) >= 2:
        st, _ = PATCH(f"/attendance-sessions/{SESSION_IDS[1]}/close",
                      {}, CENSEUR_TOKEN or ADMIN_TOKEN)
...
check(f"Création 6 présences", len(ATTENDANCE_IDS) >= 3, 201,
      f"{len(ATTENDANCE_IDS)}/6 créées")

st, att_list = GET("/attendance/?page=1&page_size=20", ADMIN_TOKEN)
check("GET /attendance/ (liste)", st == 200, st,
      f"total={att_list.get('total', '?')}")

if ATTENDANCE_IDS:
    st, _ = GET(f"/attendance/{ATTENDANCE_IDS[0]}", ADMIN_TOKEN)
    check("GET /attendance/{id} → 200", st == 200, st)

    st, _ = PATCH(f"/attendance/{ATTENDANCE_IDS[0]}",
                  {"status": "EN_RETARD"}, ADMIN_TOKEN)
...
SUBGROUP_IDS = []
for grp_name in [f"Groupe Alpha {RUN_ID}", f"Groupe Bêta {RUN_ID}", f"Groupe Gamma {RUN_ID}"]:
    time.sleep(0.1)
    st, grp = POST("/subgroups/", {
        "name": grp_name,
...
check(f"Création 3 sous-groupes", len(SUBGROUP_IDS) >= 2, 201,
...
if SUBGROUP_IDS and SERVANTS:
    ...
    for srv in SERVANTS[:6]:
        time.sleep(0.1)
        st, _ = POST(f"/subgroups/{grp_id}/members",
                     {"user_id": srv["id"]}, ADMIN_TOKEN)
        if st in (200, 201):
            members_added += 1

    check(f"POST /subgroups/{grp_id}/members ({members_added} membres)",
          members_added >= 4, 201, f"{members_added}/6")

    # Modifier le groupe
    st, _ = PATCH(f"/subgroups/{grp_id}",
                  {"name": f"Groupe Alpha Modifié {RUN_ID}",
                   "description": "Description mise à jour"}, ADMIN_TOKEN)
...
PERIOD_IDS = []
periods_data = [
    {"title": f"Cotisation Janvier {RUN_ID}",    "period_type": "MENSUEL",
     "cotisation_type": "ORDINAIRE", "amount_expected": 1000.0,
     "start_date": "2026-01-01T00:00:00", "end_date": "2026-01-31T00:00:00"},
    {"title": f"Cotisation Février {RUN_ID}",    "period_type": "MENSUEL",
     "cotisation_type": "ORDINAIRE", "amount_expected": 1000.0,
     "start_date": "2026-02-01T00:00:00", "end_date": "2026-02-28T00:00:00"},
    {"title": f"Cotisation Mars {RUN_ID}",       "period_type": "MENSUEL",
     "cotisation_type": "ORDINAIRE", "amount_expected": 1000.0,
     "start_date": "2026-03-01T00:00:00", "end_date": "2026-03-31T00:00:00"},
    {"title": f"Cotisation Trim. {RUN_ID}",      "period_type": "TRIMESTRIEL",
     "cotisation_type": "SPECIALE", "amount_expected": 5000.0,
     "start_date": "2026-01-01T00:00:00", "end_date": "2026-03-31T00:00:00"},
]
...
check(f"Création 4 périodes de cotisation", len(PERIOD_IDS) >= 2, 201,
      f"{len(PERIOD_IDS)}/4 créées")

st, p_list = GET("/cotisations/periods?page=1&page_size=10", ADMIN_TOKEN)
check("GET /cotisations/periods (liste)", st == 200 and "items" in p_list, st,
...
if PERIOD_IDS:
    ...
    for srv in SERVANTS[:10]:
        time.sleep(0.1)
        st, pmt = POST("/cotisations/payments", {
            "period_id": period_id,
            "user_id": srv["id"],
    ...
    check(f"POST /cotisations/payments ({payments_made} paiements)",
          payments_made >= 6, 201, f"{payments_made}/10")
    ...
    check("GET /cotisations/periods/{id}/payments", st == 200, st,
          f"total={_pmts_total}")

    # Bilan
    st, bilan = GET(f"/cotisations/periods/{period_id}/bilan", ADMIN_TOKEN)
    check("GET /cotisations/periods/{id}/bilan", st == 200, st,
          f"collected={bilan.get('total_collected', '?')}")

    # Modifier la période
    st, _ = PATCH(f"/cotisations/periods/{period_id}",
                  {"title": "Cotisation Janvier 2026 — Révisée"}, ADMIN_TOKEN)
...
check(f"Création 8 contributions", len(CONTRIB_IDS) >= 4, 201,
      f"{len(CONTRIB_IDS)}/8 créées")

st, c_list = GET("/contributions/?page=1&page_size=20", ADMIN_TOKEN)
check("GET /contributions/ (liste)", st == 200, st,
      f"total={c_list.get('total', '?')}")

if CONTRIB_IDS:
    st, _ = GET(f"/contributions/{CONTRIB_IDS[0]}", ADMIN_TOKEN)
    check("GET /contributions/{id}", st == 200, st)

    st, _ = PATCH(f"/contributions/{CONTRIB_IDS[0]}",
                  {"amount": 6000.0}, ECONOME_TOKEN or ADMIN_TOKEN)
...
entries_data = [
    {"amount": 25000.0,  "category": "COTISATION",  "source": "SERVANT",
     "description": "Cotisations mensuelles Janvier",
     "date": now_utc.strftime("%Y-%m-%dT%H:%M:%S")},
    {"amount": 50000.0,  "category": "DON",          "source": "EXTERNE",
     "description": "Don paroissien Février",
     "date": now_utc.strftime("%Y-%m-%dT%H:%M:%S")},
    {"amount": 15000.0,  "category": "AUTRE",        "source": "SERVANT",
     "description": "Achat encens et cierges",
     "date": now_utc.strftime("%Y-%m-%dT%H:%M:%S")},
    {"amount": 8000.0,   "category": "AUTRE",        "source": "SERVANT",
     "description": "Entretien matériel liturgique",
     "date": now_utc.strftime("%Y-%m-%dT%H:%M:%S")},
    {"amount": 12000.0,  "category": "CONTRIBUTION", "source": "SERVANT",
...
check(f"Création 5 entrées financières", len(FE_IDS) >= 3, 201,
      f"{len(FE_IDS)}/5 créées")

st, fe_list = GET("/financial-entries/?page=1&page_size=20", ADMIN_TOKEN)
check("GET /financial-entries/ (liste)", st == 200, st,
...
if FE_IDS:
    ...
    st, _ = PATCH(f"/financial-entries/{fe_id}",
    ...
    st, disc = POST(f"/financial-entries/{fe_id}/discrepancies", {
        "entry_id": fe_id,
...
st, _ = GET(f"/financial-entries/stats/summary?start_date={now_utc.strftime('%Y-%m-%dT%H:%M:%S')}&end_date={_dt(timedelta(days=30))}", COMMISSAIRE_TOKEN or ADMIN_TOKEN)
...
REPORT_IDS = []
reports_data = [
    {"type": "REUNION",  "title": f"Rapport Réunion Conseil {RUN_ID}",
    ...
    {"type": "ACTIVITE", "title": f"Compte-rendu Sortie Culturelle {RUN_ID}",
     "content": "La sortie culturelle au musée national s'est bien déroulée. " * 5,
     "report_date": now_utc.strftime("%Y-%m-%dT%H:%M:%S"),
     "location": "Musée National du Cameroun"},
    {"type": "REUNION",  "title": f"Rapport AG Trim. 1 {RUN_ID}",
...
check(f"Création {len(reports_data)} rapports", len(REPORT_IDS) >= 2, 201,
      f"{len(REPORT_IDS)}/{len(reports_data)} créés")

st, r_list = GET("/reports/?page=1&page_size=10", ADMIN_TOKEN)
check("GET /reports/ (liste)", st == 200 and "items" in r_list, st,
...
if REPORT_IDS:
    ...
    # Modifier
    st, _ = PATCH(f"/reports/{rpt_id}",
                  {"title": "Rapport Réunion Janvier — Corrigé"}, SECRETAIRE_TOKEN or ADMIN_TOKEN)
...
st, _ = POST("/reports/", {
    "type": "REUNION", "title": "Test admin", "content": "x" * 50,
    "report_date": now_utc.strftime("%Y-%m-%dT%H:%M:%S"), "location": "ici"
}, ADMIN_TOKEN)
...
check(f"Création 3 dossiers disciplinaires", len(CASE_IDS) >= 2, 201,
      f"{len(CASE_IDS)}/3 créés")

st, d_list = GET("/discipline/?page=1&page_size=10", ADMIN_TOKEN)
check("GET /discipline/ (liste)", st == 200, st,
      f"total={d_list.get('total', '?')}")
...
if CASE_IDS:
    ...
    st, _ = POST(f"/discipline/{case_id}/sanctions", {
    ...
    st, _ = PATCH(f"/discipline/{case_id}",
                  {"status": "EN_COURS"}, CENSEUR_TOKEN or ADMIN_TOKEN)
...
sc_events = [
    {"title": f"Football inter-groupes {RUN_ID}",
     ...
     "date": FUTURE_2, "start_time": "08h00", "end_time": "18h00",
    ...
    {"title": f"Visite Musée National {RUN_ID}",
     ...
     "date": FUTURE_3, "start_time": "09h00", "end_time": "16h00",
     "location": "Musée National", "max_participants": 25},
    {"title": f"Basket-ball {RUN_ID}",
     "description": "Compétition de basket-ball en équipes.",
     "event_type": "JOURNEE_SPORTIVE",
     "date": FUTURE_4, "start_time": "14h00", "end_time": "18h00",
     "location": "Gymnase paroissial", "max_participants": 20},
]
...
check(f"Création {len(sc_events)} événements sport & culture", len(SC_EVENT_IDS) >= 2, 201,
      f"{len(SC_EVENT_IDS)}/{len(sc_events)} créés")

st, sc_list = GET("/sport-culture/events?page=1&page_size=10", ADMIN_TOKEN)
check("GET /sport-culture/events (liste)", st == 200 and "items" in sc_list, st,
...
if SC_EVENT_IDS:
    ...
    for srv in SERVANTS[:6]:
        time.sleep(0.1)
        st, _ = POST(f"/sport-culture/events/{sc_id}/register", {
            "servant_id": srv["id"],
        }, SPORT_TOKEN or ADMIN_TOKEN)
        if st in (200, 201):
            part_added += 1
    check(f"Inscription participants ({part_added})", part_added >= 4, 201,
          f"{part_added}/6")

    # Créer équipes
    if len(SERVANTS) >= 4:
        st, team = POST(f"/sport-culture/events/{sc_id}/teams", {
    ...
    st, _ = POST(f"/sport-culture/events/{sc_id}/results", {
...
training_data = [
    {"title": f"Formation Liturgie Base {RUN_ID}",
     ...
     "date": FUTURE_1, "start_time": "09h00", "end_time": "12h00",
     "duration_minutes": 180, "level": "DEBUTANT",
     "location": "Salle de catéchèse", "trainer_id": ADMIN_ID,
     "max_participants": 20},
    {"title": f"Formation Chant Liturgique {RUN_ID}",
     "description": "Initiation au chant sacré et au grégorien simplifié.",
     "date": FUTURE_2, "start_time": "14h00", "end_time": "17h00",
     "duration_minutes": 180, "level": "TOUS",
     "location": "Chorale", "trainer_id": ADMIN_ID,
     "max_participants": 30},
    {"title": f"Formation Avancée Cérémoniaire {RUN_ID}",
     "description": "Techniques avancées pour les cérémonies pontificales.",
     "date": FUTURE_3, "start_time": "08h00", "end_time": "13h00",
     "duration_minutes": 300, "level": "AVANCE",
     "location": "Cathédrale", "trainer_id": ADMIN_ID,
     "max_participants": 10},
]
...
check(f"Création {len(training_data)} sessions de formation", len(TRAINING_IDS) >= 2, 201,
      f"{len(TRAINING_IDS)}/{len(training_data)} créées")

st, tr_list = GET("/training/sessions?page=1&page_size=10", ADMIN_TOKEN)
check("GET /training/sessions (liste)", st == 200 and "items" in tr_list, st,
...
if TRAINING_IDS:
    ...
    for srv in SERVANTS[:6]:
        time.sleep(0.1)
        st, _ = POST(f"/training/sessions/{tr_id}/register", {
            "servant_id": srv["id"],
        }, LITURGIE_TOKEN or ADMIN_TOKEN)
        if st in (200, 201):
            part_count += 1
    check(f"Inscription {part_count} participants à la formation", part_count >= 3, 201,
          f"{part_count}/6")

    # Modifier la session
    st, _ = PATCH(f"/training/sessions/{tr_id}",
...
if TRAINING_IDS:
    st, mat = POST("/training/materials", {
...
ITEM_IDS = []
items_data = [
    {"name": f"Encensoir {RUN_ID}", "category": "ENCENSOIR",
     "quantity": 2, "condition": "BON", "location": "Sacristie principale"},
    {"name": f"Calice doré {RUN_ID}", "category": "CALICE",
     "quantity": 1, "condition": "BON", "location": "Armoire sécurisée"},
    {"name": f"Patène {RUN_ID}",    "category": "PATENE",
     "quantity": 3, "condition": "BON", "location": "Sacristie principale"},
    {"name": f"Cierge {RUN_ID}",    "category": "CIERGE",
     "quantity": 4, "condition": "BON", "location": "Penderie liturgique"},
    {"name": f"Nappe {RUN_ID}",     "category": "NAPPE",
     "quantity": 2, "condition": "A_NETTOYER", "location": "Placard"},
    {"name": f"Aubes {RUN_ID}",     "category": "AUBE",
     "quantity": 10, "condition": "BON", "location": "Blanchisserie"},
]
...
check(f"Création {len(items_data)} articles de matériel", len(ITEM_IDS) >= 4, 201,
      f"{len(ITEM_IDS)}/{len(items_data)} créés")

st, it_list = GET("/material/items/?page=1&page_size=20", ADMIN_TOKEN)
check("GET /material/items/ (liste)", st == 200 and "items" in it_list, st,
...
if ITEM_IDS:
    ...
    st, _ = PATCH(f"/material/items/{item_id}",
    ...
    # Ajouter maintenance
    st, _ = POST(f"/material/items/{item_id}/maintenance", {
...
tasks_data = [
    {"title": "Nettoyage sacristie principale",
     ...
     "task_type": "NETTOYAGE",
     "scheduled_date": FUTURE_1, "scheduled_time": "15h00",
     "location": "Sacristie"},
    {"title": "Lavage des aubes",
     "description": "Lavage et repassage des aubes de la semaine.",
     "task_type": "LAVAGE",
     "scheduled_date": FUTURE_2, "scheduled_time": "10h00",
...
check(f"Création {len(tasks_data)} tâches", len(TASK_IDS) >= 1, 201,
      f"{len(TASK_IDS)}/{len(tasks_data)}")

if TASK_IDS and SERVANTS:
    task_id = TASK_IDS[0]
    # Assigner un servant à la tâche
    st, _ = POST(f"/material/cleaning-tasks/{task_id}/assign", {
...
for poste_key, slug_val in list(POSTE_SLUGS.items())[:4]:
    ...
    time.sleep(0.1)
    st, action = POST(f"/poste/{slug_val}/actions", {
        "category": cat,
...
check(f"Création actions de poste ({len(ACTION_IDS)})", len(ACTION_IDS) >= 2, 201,
...
if ACTION_IDS:
    ...
    check(f"PATCH /poste/{slug_val}/actions/{action_id}/publish", st in (200, 204, 404), st)
...
check("GET /communication/me (mes notifs)", st == 200, st,
...
for i, srv in enumerate(SERVANTS[:6]):
    time.sleep(0.1)
    notification_types = ["GENERAL", "COTISATION", "DISCIPLINE",
                          "GENERAL", "COTISATION", "DISCIPLINE"]
    notif_type = notification_types[i % len(notification_types)]
    st, notif = POST("/communication/notify", {
        "recipient_id": srv["id"],
...
check(f"POST /communication/notify ({len(NOTIF_IDS)} envoyées)", len(NOTIF_IDS) >= 4, 201,
      f"{len(NOTIF_IDS)}/6")

# Notification broadcast (tous les servants)
st, _ = POST("/communication/broadcast", {
...
if SERVANT_TOKEN_1:
    st, _ = PUT("/communication/me/preferences", {
        "notification_type": "GENERAL",
        "email_enabled": False,
...
# Créer un template
st, ss = POST("/sunday-schedule/", {
    "title": "Planning Dimanche — Test E2E",
    "sunday_date": FUTURE_2,
    "masses": [
        {"mass_time": "07h30", "mass_type": "ORDINAIRE",
         "main_celebrant": "Père Jean", "notes": "Messe de semaine"},
        {"mass_time": "10h00", "mass_type": "SOLENNELLE",
         "main_celebrant": "Père Paul", "notes": "Grand-messe"},
    ],
}, CLASSEMENT_TOKEN or ADMIN_TOKEN)
SS_ID = ss.get("id", "")
check("POST /sunday-schedule/ (créer template)", st in (200, 201, 422), st,
...
if SERVANT_ID_1:
    ...
    check("GET /dossier/{user_id} (admin) → 200", st == 200, st,
...
if SERVANT_TOKEN_1:
    ...
    st, _ = POST("/reports/", {
        "type": "REUNION", "title": "Test", "content": "x" * 50,
        "report_date": now_utc.strftime("%Y-%m-%dT%H:%M:%S"), "location": "ici"
    }, SERVANT_TOKEN_1)
    ...
    st, _ = POST("/cotisations/periods", {
        "title": "Test", "period_type": "MENSUEL",
        "cotisation_type": "ORDINAIRE", "amount_expected": 100.0,
        "start_date": "2026-06-01T00:00:00", "end_date": "2026-06-30T00:00:00",
    }, SERVANT_TOKEN_1)
...
check("GET /api/v1/version", st == 200, st,
...
check("POST /auth/verify-reset-code (code invalide → 400/404/422)", st in (400, 404, 422), st)
...
if SESSION_IDS and SERVANTS:
    ...
    check("POST /attendance-sessions/{id}/init-roll-call", st in (200, 201, 400, 404, 409), st)

    # Rapport de session
    st, _ = POST("/attendance-sessions/report", {
        "session_id": s_id,
        "format": "json",
    }, CENSEUR_TOKEN or ADMIN_TOKEN)
...
if ITEM_IDS and SERVANTS:
    ...
    st, ct = POST("/material/cleaning-tasks", {
        "item_id": ITEM_IDS[0],
    ...
    if CLEANING_TASK_IDS:
        ...
        st, _ = PATCH(f"/material/cleaning-tasks/{ct_id}",
                      {"description": "Nettoyage complet — mis à jour"}, INTENDANT_TOKEN or ADMIN_TOKEN)
        check("PATCH /material/cleaning-tasks/{id}", st in (200, 404, 422), st)

        # Compléter la tâche
        st, _ = POST(f"/material/cleaning-tasks/{ct_id}/complete",
                     {"completion_notes": "Nettoyage effectué le " + now_utc.strftime("%d/%m/%Y")},
                     SERVANT_TOKEN_1 or ADMIN_TOKEN)
        check("POST /material/cleaning-tasks/{id}/complete", st in (200, 201, 400, 404), st)

    # Maintenance d'un article
    st, maint = POST(f"/material/items/{ITEM_IDS[0]}/maintenance", {
        "maintenance_type": "NETTOYAGE",
...
if FE_IDS:
    ...
    st, _ = POST(f"/financial-entries/{fe_id2}/verify", {
    ...
    # Rapport d'audit
    st, audit = POST("/financial-entries/audit/report", {
    ...
    st, _ = GET(f"/financial-entries/export/pdf?start_date={_dt(-timedelta(days=30))}&end_date={_dt(timedelta(days=1))}",
                COMMISSAIRE_TOKEN or ADMIN_TOKEN)
...
if REPORT_IDS and SECRETAIRE_TOKEN:
    ...
    if REPORT_IDS:
        st, _ = POST(f"/reports/{REPORT_IDS[0]}/attachments", {
...
if CASE_IDS and SERVANTS:
    ...
    st, _ = POST(f"/discipline/{case_id2}/convoke", {
    ...
    st, _ = POST(f"/discipline/{case_id2}/hearing", {
    ...
    st, _ = POST(f"/discipline/{case_id2}/verdict", {
    ...
    if len(CASE_IDS) >= 2:
        st, _ = POST(f"/discipline/{CASE_IDS[1]}/dismiss",
                     {"dismiss_reason": "Manque de preuves suffisantes."}, CENSEUR_TOKEN or ADMIN_TOKEN)
...
if _read_notif_id:
    st, _ = POST("/communication/me/read", {"notification_ids": [_read_notif_id]}, SERVANT_TOKEN_1 or ADMIN_TOKEN)
else:
    import uuid as _uuid_mod
    st, _ = POST("/communication/me/read", {"notification_ids": [str(_uuid_mod.uuid4())]}, SERVANT_TOKEN_1 or ADMIN_TOKEN)
...
if PERIOD_IDS:
    ...
    st, _ = PATCH(f"/cotisations/periods/{PERIOD_IDS[0]}",
                  {"title": f"Cotisation Janvier Modifiée {RUN_ID}"}, ADMIN_TOKEN)
...
if SC_EVENT_IDS:
    ...
    st, result = POST(f"/sport-culture/events/{sc_id2}/results", {
    ...
    st, _ = POST("/sport-culture/report", {
...
if TRAINING_IDS:
    ...
    if TR_MAT_IDS:
        ...
        st, _ = PATCH(f"/training/materials/{mat_id}",
                      {"title": f"Guide Avancé — Révisé {RUN_ID}"}, LITURGIE_TOKEN or ADMIN_TOKEN)
...
st, ss_ord = POST("/sunday-schedule/generate/ordinary", {
    "week_start": FUTURE_2,
...
st, ss_exc = POST("/sunday-schedule/generate/exceptional", {
    "week_start": FUTURE_3,
    ...
    if st == 200 and isinstance(w_list_pub, dict):
        ...
        if items:
            ...
            if ws_first_id:
                ...
                check("PATCH /weekly-schedule/{id}/publish", st in (200, 204, 400, 404), st)
            else:
                check("PATCH /weekly-schedule/{id}/publish", True, 200, "SKIP: aucun planning")
        else:
            check("PATCH /weekly-schedule/{id}/publish", True, 200, "SKIP: liste vide")
    else:
        check("PATCH /weekly-schedule/{id}/publish", True, 200, "SKIP: liste non disponible")
...
st, new_parent = POST("/admin/users/parent", {
...
if st == 200 and isinstance(cl_list, dict):
    ...
    if cl_items:
        ...
        if cl_id:
            # Avancer d'un niveau
            st, _ = POST(f"/classements/{cl_id}/advance", {
                "new_level": "SENIOR",
...
st, meeting = POST("/responsables/council-meetings", {
...
if meeting_id and SERVANTS:
    st, _ = POST(f"/responsables/council-meetings/{meeting_id}/attendance", {
        "attendances": [{"responsable_id": SERVANTS[0]["id"], "is_present": True}],
    }, DELEGUE_TOKEN or ADMIN_TOKEN)
    check("POST /responsables/council-meetings/{id}/attendance", st in (200, 201, 400, 404, 422), st)
...
if EVENT_IDS:
    ...
    st, _ = POST(f"/events/{ev_id2}/check-in", {
        "servant_id": SERVANTS[0]["id"] if SERVANTS else None,
    }, SERVANT_TOKEN_1 or ADMIN_TOKEN)
...
if SERVANTS:
    ...
    # Stats d'un servant
    st, _ = GET(f"/contributions/servant/{srv0_id}/stats?start_date={_dt(-timedelta(days=365))}&end_date={_dt(timedelta(days=1))}",
                ADMIN_TOKEN)
    ...
    st, _ = POST("/contributions/report", {
