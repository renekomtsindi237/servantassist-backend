...
class IContributionRepository(Protocol):
    ...
    async def list(self, *, page: int = 1, page_size: int = 20, **filters) -> Tuple[List[Contribution], int]: ...

    async def get_servant_contributions(self, user_id: UUID) -> List[Contribution]: ...

    async def get_monthly_contributions(self, year: int, month: int) -> List[Contribution]: ...
