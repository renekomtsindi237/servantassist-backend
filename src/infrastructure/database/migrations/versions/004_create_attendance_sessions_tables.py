...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "session_time", sa.String(10), nullable=False, server_default="07h30"
        ),
        sa.Column(
            "location", sa.String(100), nullable=False, server_default="Sacristie"
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["session_id"], ["attendance_sessions.id"], ondelete="CASCADE"
        ),
    ...
    op.create_index(
        "idx_attendance_sessions_date", "attendance_sessions", ["session_date"]
    )
    op.create_index(
        "idx_attendance_sessions_conducted_by", "attendance_sessions", ["conducted_by"]
    )

    op.create_index(
        "idx_attendance_records_session_id", "attendance_records", ["session_id"]
    )
    op.create_index(
        "idx_attendance_records_servant_id", "attendance_records", ["servant_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "idx_attendance_records_servant_session", table_name="attendance_records"
    )
    ...
    op.drop_index(
        "idx_attendance_sessions_conducted_by", table_name="attendance_sessions"
    )
