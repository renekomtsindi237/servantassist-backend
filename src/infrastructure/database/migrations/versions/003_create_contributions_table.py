...
def upgrade() -> None:
    """Create contributions table for ECONOME module."""
    ...
    "month >= 1 AND month <= 12",
     name="check_month_valid"),
        sa.CheckConstraint(
    "year >= 2020 AND year <= 2100",
     name="check_year_valid"),
        sa.CheckConstraint(
            "week_number IS NULL OR (week_number >= 1 AND week_number <= 4)",
            name="check_week_number_valid",
        ),
        sa.CheckConstraint(
            "(payment_mode = 'HEBDOMADAIRE' AND week_number IS NOT NULL) OR (payment_mode = 'MENSUEL' AND week_number IS NULL)",
    ...
    op.create_index(
    "idx_contributions_servant_id",
    "contributions",
     ["servant_id"])
    op.create_index("idx_contributions_month_year",
                    "contributions", ["month", "year"])
    op.create_index(
    "idx_contributions_payment_date",
    "contributions",
     ["payment_date"])
    op.create_index(
    "idx_contributions_recorded_by",
    "contributions",
     ["recorded_by"])
    op.create_index(
    "idx_contributions_payment_mode",
    "contributions",
     ["payment_mode"])
